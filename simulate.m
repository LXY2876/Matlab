 X=[3 4 1;3 8 1;4 3 1;4 9 1;5 4 1;5 6 1;5 8 1;6 3 1;6 9 1;7 4 1;7 8 1];                          
...
while (sum(sum(L))~=270)
   task=design(x1,y1,d,L,X); 
   i=2;%task(2)=0;task(3)=0;task(4)=1;
   a=d;%a用于储存方向；
   if task(1)~=1
       L=random(L);
     imagesc(L);        
    saveas(gcf,strcat('D:\ruanjian\MATLABfiles',int2str(8),'1.jpg'));%saveas函数是为了让图片显示并保存
   else
